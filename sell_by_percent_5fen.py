...
def process_result_test(df, sell, peak_persent, run_index, result):
    ...
    for i in range(df['日期'].size):
        if i > 0:
            current_price = df.iloc[i, 2]
            if (df.iloc[i - 1, 3] < df.iloc[i - 1, 4]) \
                    and (df.iloc[i - 1, 4] < df.iloc[i - 1, 5]) \
                    and (df.iloc[i - 1, 5] < df.iloc[i - 1, 6]) \
                    and (df.iloc[i, 3] > df.iloc[i, 4]):
                mairu = 10000000 / current_price
            ...
            # 这里是卖出策略
            elif peak_persent > 0 and (
                    (df.iloc[i, 3] > df.iloc[i, 5]) and (df.iloc[i, 5] > df.iloc[i, 6]) and (zonggushu > 0)):
                if (peak_price > 0.001) and (current_price < peak_price * (1 - peak_persent)):
                    ...
                    (df.iloc[i - 1, 3] > df.iloc[i - 1, 4]) and (df.iloc[i - 1, 4] > df.iloc[i - 1, 5]) and (
                    df.iloc[i - 1, 5] > df.iloc[i - 1, 6]) and (df.iloc[i, 3] < df.iloc[i, 4]) and (zonggushu > 0)):
                maichu = zonggushu / sell
                maichu = maichu - maichu % 100
                if maichu < 100:
                    maichu = 100
    ...
    result.loc[run_index, '交易天数'] = df['日期'].size / 4 / 12
    if zonggushu > 0:
        result.loc[run_index, '平均股价'] = zongjine / zonggushu
    else:
        result.loc[run_index, '平均股价'] = 0.001
    result.loc[run_index, '当前股价'] = df.iloc[df['日期'].size - 1, 2]
...
def get_average_line(df: pd.DataFrame, days):
    ...
    data_return = pd.DataFrame(columns=['日期', '时间', col_name])
    for i in range(df['收盘'].size - days + 1):
        data_return.loc[i, '日期'] = df.iloc[i + days - 1, 0]
        data_return.loc[i, '时间'] = df.iloc[i + days - 1, 1]
        data_return.loc[i, col_name] = df.iloc[i:i + days, 5].sum() / df.iloc[i:i + days, 5].size
    return data_return


def process_data(filename, line1, line2, line3, line4, sell, peak_persent, run_index, result):
    filepath = '5fen/' + filename

    df = pd.read_csv(filepath, header=1, low_memory=False)

    df = df.iloc[:-1]
    try:
        line1_average = get_average_line(df, line1)
        line2_average = get_average_line(df, line2)
        line3_average = get_average_line(df, line3)
        line4_average = get_average_line(df, line4)

        df = df[['日期', '时间', '收盘']]

        df = pd.merge(df, line1_average, on=['日期', '时间'])
        df = pd.merge(df, line2_average, on=['日期', '时间'])
        df = pd.merge(df, line3_average, on=['日期', '时间'])
        df = pd.merge(df, line4_average, on=['日期', '时间'])
